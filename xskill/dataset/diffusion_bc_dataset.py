...
class KitchenRealDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        data_dirs,
        proto_dirs,
        ...
        # normalized_train_data = dict()
        for key, data in train_data.items():
            if "images" in key or key in self.unnormal_list:
                pass
            else:
                stats[key] = get_data_stats(data)
    ...
    def load_proto_and_to_tensor(self, vid, is_paired=False, is_lookup=False):
        directories = vid.split(os.path.sep)
        proto_path = osp.join(self.proto_dirs, directories[-2], directories[-1])
    ...
    def load_data(self, train_data):
        ...
        # vid = list(self._dir_tree.values())[0]
        vid = []
        for sub_folder in list(self._dir_tree.values()):
            vid.extend(sub_folder)
        print("loading data")
        ...
        for j, v in tqdm(enumerate(vid), desc="Loading data", disable=not self.verbose):
            if self.obs_image_based:
                wrist_images = self.load_images(v + '/wrist')
                overhead_images = self.load_images(v + '/overhead')
                train_data["wrist_images"].append(wrist_images)
                train_data["overhead_images"].append(overhead_images)
        
            vid_len = wrist_images.shape[0]

            train_data["obs"].append(self.load_state_and_to_tensor(v))
            if self.prototype_snap:
                proto_data, proto_snap = self.load_proto_and_to_tensor(v, is_paired=(v in paired_set), is_lookup=(v in lookup_set))
                proto_data = torch.zeros((vid_len, proto_data.shape[1]))
                proto_snap = torch.zeros((vid_len, proto_snap.shape[1]))
                train_data["proto_snap"].append(proto_snap)
            else:
                proto_data = self.load_proto_and_to_tensor(v)
    ...
    def __getitem__(self, idx):
        ...
        if self.obs_image_based:
            ...
            nsample["wrist_images"] = nsample["wrist_images"][: self.obs_horizon, :]

            nsample["overhead_images"] = self.transform_images(nsample["overhead_images"])
            nsample["overhead_images"] = nsample["overhead_images"][: self.obs_horizon, :]
