...
class BCTaskState(DefaultTask):

    def eval(self, trainer, global_step):
        losses = []
        action_l2, action_lsig, action_l2_norm = [], [],  []
        for batch in self.test_loader:
            ...
            with torch.no_grad():
                ...
                losses.append(loss.item())
                # compare predicted actions versus GT
                pred_actions = trainer.model.get_actions(obs)
                
                # calculate l2 loss between pred_action and action
                l2_delta = torch.square(mask * (pred_actions - actions))
                l2_delta = l2_delta.sum((1, 2)) / mask.sum((1, 2))
                
                l2_norm = torch.square(mask * (actions))
                l2_norm = l2_norm.sum((1, 2)) / mask.sum((1, 2))
                ...
                action_l2.append(l2_delta.mean().item())
                action_lsig.append(lsig.mean().item())
                action_l2_norm.append(l2_delta.mean().item()/l2_norm.mean().item())
        
        print(losses)
        mean_val_loss = np.mean(losses)
        ac_l2, ac_lsig, ac_norm = np.mean(action_l2), np.mean(action_lsig), np.mean(action_l2_norm)
        print(f'Step: {global_step}\tVal Loss: {mean_val_loss:.4f}')
        print(f'Step: {global_step}\tAC L2={ac_l2:.2f}\tAC LSig={ac_lsig:.2f} \tAC Norm={ac_norm:.2f}')
        
        if wandb.run is not None:
            wandb.log({'eval/task_loss': mean_val_loss,
                       'eval/action_l2': ac_l2,
                       'eval/action_lsig': ac_lsig,
                       'eval/action_norm': ac_norm}, step=global_step)
        return ac_l2
