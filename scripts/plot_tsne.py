...
def plot(x, y, divider, colors_used, cfg):
    ...
    plt.scatter(robot_x[:, 0], robot_x[:, 1], c=robot_colors, s=100, marker='o', edgecolors='black', linewidths=1)
    plt.scatter(human_x[:, 0], human_x[:, 1], c=human_colors, s=100, marker='P', edgecolors='black', linewidths=1)
...
def plot_tsne(cfg):
    ...
    colors = {
        ...
        'kettle_light switch': 'lightcyan',
        'kettle_bottom burner': 'lightcoral',
    ...
    for task1, task2 in human_tasks_interested:
        human_task_frame_dict[f'{task1}_{task2}'] = []
    ...
    for eps_num in [17, 56, 45, 31, 30, 19, 48, 38, 59, 10, 5, 58, 20, 57, 6, 42]:
        ...
        for task1, task2 in human_tasks_interested:
            for i in range(len(human_moved_obj)):
                ...
                if human_moved_obj[start, task_to_idx[task1]] and human_moved_obj[start, task_to_idx[task2]] and human_moved_obj[end, task_to_idx[task1]] and human_moved_obj[end, task_to_idx[task2]]:
                    human_task_frame_dict[f'{task1}_{task2}'].append(human_data[i])
    ...
    robot_len = len(x)
    for task in human_tasks_interested:
        if len(human_task_frame_dict[f'{task[0]}_{task[1]}']) == 0:
            continue
        print("im here")
        
        sample = random.sample(human_task_frame_dict[f'{task[0]}_{task[1]}'], min(50, len(human_task_frame_dict[f'{task[0]}_{task[1]}'])))
        x.extend(sample)
        y.extend([colors[f'{task[0]}_{task[1]}']]*len(sample))
        ...
        colors_used[f'{task[0]}_{task[1]}'] = colors[f'{task[0]}_{task[1]}']
    
    tsne = TSNE(n_components=2, random_state=0, perplexity=50, n_iter=10000)
    tsne_obj = tsne.fit_transform(np.array(x))

    
    np.save(os.path.join(f"tsne_{cfg.ckpt}_hard.npy"), tsne_obj)
    np.save(os.path.join(f"y_{cfg.ckpt}_hard.npy"), y)
    # if not os.path.exists(f"tsne_{cfg.ckpt}_hard.npy"):
    #     np.save(os.path.join(f"tsne_{cfg.ckpt}_hard.npy"), tsne_obj)
    #     np.save(os.path.join(f"y_{cfg.ckpt}_hard.npy"), y)
    # else:
    #     tsne_obj = np.load(os.path.join(f"tsne_{cfg.ckpt}_hard.npy"))
    #     y = np.load(os.path.join(f"y_{cfg.ckpt}_hard.npy"))
    plot(tsne_obj, y, robot_len, colors_used, cfg)
    # save tsne_obj and y to a file
    # np.save(os.path.join(f"tsne_{cfg.ckpt}_hard.npy"), tsne_obj)
    # np.save(os.path.join(f"y_{cfg.ckpt}_hard.npy"), y)
    ...
    # plt.savefig(f"tsne_hard_finetuned.png")
    # plt.show()
    # breakpoint()
