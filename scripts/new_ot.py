...
def label_dataset(cfg: DictConfig):
    with open("/share/portal/pd337/xskill/task_completions.json", "r") as f:
    ...
    # for demo_type in ["twohands", "human"]:
        ...
        for folder_path in tqdm(all_folders, disable=not cfg.verbose):
            ...
            if len(robot_list) < 4:
                ...
                task_list = task_completions[int(folder_path)][:2]
            else:
                robot_list = [{
                    'data_folder': data_folder,
                    'start_idx': robot_list[0]['start_idx'],
                ...
                task_list = [task_completions[int(folder_path)][:2], task_completions[int(folder_path)][2:4]]
            
            for k, (robot_dict, human_dict) in enumerate(zip(robot_list, human_list)):
                task_l.append(task_list[k])
