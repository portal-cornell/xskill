        ...
        for demo_type in [human_type, 'robot']:
            ...
            for speed in speeds[demo_type]:
                ...
                for seed in eval_eps:
                    ...
                    num_unspec_attempt = get_attempted_unspecified_tasks(task_list, initial_obs, final_obs)
                    num_unspecified_tasks += num_unspec_attempt
                    task_completion_rate = num_completed / 4  # Assuming 4 tasks per episode
                    task_completion_rates_list.append(task_completion_rate)

                    misfire_rate = num_unspec_attempt / 3  # Assuming 3 unspecified tasks per episode
                ...
                print(misfire_rates_list)
                print(std_dev_misfire)
                ...
                result_dict[demo_type][f'{ckpt_num}'][f'{speed}']['num-unspecified'] = num_unspecified_tasks / (3*len(eval_eps))
                result_dict[demo_type][f'{ckpt_num}'][f'{speed}']['std'] = std_dev_task_completion_rate / sqrt(len(eval_eps))
                result_dict[demo_type][f'{ckpt_num}'][f'{speed}']['std_miss'] = std_dev_misfire / sqrt(len(eval_eps))
                print(result_dict)
